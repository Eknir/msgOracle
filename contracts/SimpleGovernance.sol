...
contract SimpleGovernance {
    ...
    event LogSetVotingWindow(uint256 votingWindow);
    ...
    @param governers the initial governers who can vote and start new proposals
    ...
    */
    constructor(address[] memory governers, uint256 _governersPercentageNeeded, uint256 _votingWindow) internal {
        _setGovernersPercentageNeeded(_governersPercentageNeeded);
        for(uint256 i = 0; i <= governers.length; i++) {
            _addGoverner(governers[i]);
        }
    ...
    */
    function isValidProposal(bytes32 proposal) public view returns(bool) {
        if(
            proposalRegistry[proposal].startedAt != 0 &&
            proposalRegistry[proposal].startedAt.add(votingWindow) <= now &&
    ...
    -nonce just a number to ensure uniqueness
    */
    function newProposal(bytes32 proposal) public onlyGoverner {
        require(proposalRegistry[proposal].startedAt == 0, "SimpleGovernance: proposal existing");
        proposalRegistry[proposal] = Proposal(now, 0);
    ...
    */
    function vote(bytes32 proposal) public onlyGoverner {
        ...
        require(proposalRegistry[proposal].startedAt != 0, "SimpleGovernance: proposal not started");
    ...
    */
    ...
    /**
    @notice sets the votingWindow when a valid proposal is present
    @dev A proposal can be made and voted upon via the functions newProposal and vote (respectively).
    @param newVotingWindow Maximum time in seconds between creating a proposal and implementing the change of the proposal.
    The bytes32 proposal value (functions: newProposal and vote) is keccak256(abi.encodePacked("CVW", newVotingWindow, proposalNonce))
    @param proposalNonce Value needed to ensure always-unique proposal identifiers.
    */
    function changeVotingWindow(uint256 newVotingWindow, bytes32 proposalNonce) public {
        require(isValidProposal(keccak256(abi.encodePacked(
            "CVW",
            newVotingWindow,
            proposalNonce))),
        ...
        _setVotingWindow(newVotingWindow);
    }
    ...
    /**
    @notice Internal function which sets a new votingWindow.
    @dev Called in this contract functions: constructor and changeVotingWindow.
     */
    function _setVotingWindow(uint256 _votingWindow) internal {
        votingWindow = _votingWindow;
        emit LogSetVotingWindow(_votingWindow);
    }
